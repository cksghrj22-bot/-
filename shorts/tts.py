...
SYNTH_FIXES: dict[str, str] = {
    "CS컬": "씨에스컬",      # CS컬 → [씨에스컬] (자막 'CS컬' 유지) — 이찬호 2026-07-23 중단발
    "cs컬": "씨에스컬",
    "C컬": "씨컬",          # 영문 'C'컬 → [씨컬] (자막은 'C컬' 유지) — 이찬호 2026-07-23
    ...
    "거림이었": "거림 이었",  # 찰랑거림이었 = EL이 '찰랑거림이/었'로 잘못 끊음 → '찰랑거림 이었으니까요'로 띄워 '이었'을 뒤에 붙임(이찬호 2026-07-22 재지적)
    "드러낼 뿐": "드러낼뿐",  # '드러낼 뿐' 사이 쉼이 너무 김 → 공백 제거해 붙여읽기(이찬호 2026-07-22)
    "단발은": "단바른",      # 단발은 = [단바른] 연음을 EL이 끊어 어색 → 소리대로 (이찬호 2026-07-22)
    "안 낸 것 같은데": "안낸것 같은데",  # 원랭스: 다 띄우면 EL이 뚝뚝 끊어 어색 → '안낸것'으로 붙여 흐르게(자막은 '안 낸 것')(이찬호 2026-07-22)
    "끝의": "끄테",          # 끝의 = 연음 [끄테]로. EL이 [끄데]로 뭉개는 것 방지(이찬호 2026-07-22 박제: ㅌ 살려 연음)
    "끝에": "끄테",          # 끝에 = 같은 규칙 [끄테](끄데 금지)
    "같이": "가치",          # 같이 = 구개음화 [가치](ㅌ+ㅣ→ㅊ). EL이 [갇이]로 딱딱하게 읽음 → 소리대로(이찬호 2026-07-23 검사 B 채택, prompts/07)
}
