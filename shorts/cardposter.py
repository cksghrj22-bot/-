...
def _lf_expand() -> str:
    ...
    # 옆머리 = 머리에 붙은 크고 둥근 물결컬(가로 볼륨). 지렁이/벌레다리 X.
    def _wave(x, y, s):
        a = 22 * s   # 물결 진폭(크고 둥글게, 넓게)
        # 머리에서 시작 → 큰 둥근 물결 2번 → 턱 라인
        return (f'M{x} {y} '
                f'c {a} 10, {a} 40, 0 50 '
                f'c {-a} 10, {-a} 40, 0 50')
    strands = (
        _wave(182, 96, 1) + " " + _wave(196, 108, 1)      # 오른쪽 2가닥(머리에 붙어)
        + " " + _wave(118, 96, -1) + " " + _wave(104, 108, -1)  # 왼쪽 2가닥
    )
    return f'''<svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
