...
def _lf_expand() -> str:
    ...
    # 옆머리 = 바깥으로 부드럽게 벌어지는 물결컬(가로 볼륨). 지네발 X, 곡선으로 넓게.
    # 오른쪽: 관자놀이→바깥으로 불룩→턱, 좌우 대칭. 살짝 물결 결.
    strands = (
        # 오른쪽 바깥 가닥(크게 벌어짐)
        'M190 86 C 244 116, 236 138, 246 162 S 214 190, 202 208 '
        # 오른쪽 안쪽 가닥
        'M186 94 C 220 120, 214 150, 220 168 S 198 192, 190 205 '
        # 왼쪽 바깥 가닥(대칭)
        'M110 86 C 56 116, 64 138, 54 162 S 86 190, 98 208 '
        # 왼쪽 안쪽 가닥
        'M114 94 C 80 120, 86 150, 80 168 S 102 192, 110 205'
    )
    return f'''<svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg">
