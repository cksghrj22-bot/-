...
def square_detail_spec() -> DetailSpec:
    return DetailSpec(
        series="사각형 얼굴 단발 공식",
        foot="@차노쌤 · 사각도 예뻐요 — 안 예쁘다는 생각을 버리면 다 자유로워져요",
        cover=square_spec(),   # 1번=처음 그 사각형 카드(공식 6칸) 그대로
        cards=[
            DetailCard("각진 곳에 길이를 맞춘다", "길이는\n각을 비켜요", "각에 라인을 두지 마라", "sq_avoid",
                       "커트 라인을 각진 곳에 딱 맞추면\n그 각이 오히려 더 살아나요.\n길이 기준점을 각에서 살짝 비켜야\n각이 강조되지 않아요."),
            DetailCard("곡선은 셀수록 부드럽다", "곡선도\n과하면 역효과", "곡선은 '적당히'", "sq_round",
                       "각의 선을 곡선으로 감싸되,\n곡선이 너무 강하면 오히려\n직선이 대비돼 턱이 더 부각돼요.\n곡선은 적당함이 핵심이에요."),
            DetailCard("옆 볼륨은 살릴수록 좋다", "모발로 생긴 각은\n지워요", "가로선 확장을 막는다", "sq_jawcurl",
                       "얼굴 자체의 각은 의사 영역이에요.\n우리는 모발이 부풀어 생기는 각만 손봐요.\n옆이 부풀면 가로선이 확장되니\n커트와 매직으로 눌러 정리해요."),
            DetailCard("가벼울수록 예쁘다", "머리는\n불투명하게", "속이 비치면 각이 산다", "hair_texture",
                       "속이 비쳐 보이면 얼굴 선이 강조돼요.\n루트시닝과 질감 테크닉으로\n머리를 불투명하게 채우면\n얼굴의 각진 선이 덜 보여요."),
            DetailCard("가리면 감춰진다", "가릴 거면\n티 안 나게", "티 나면 더 강조된다", "sq_bang",
                       "헤어로 단점을 가릴 거면\n티가 안 나게 자연스러워야 해요.\n'가렸다'는 게 의도적으로 보이면\n그 단점이 오히려 더 강조돼요."),
            DetailCard("무조건 단발", "차라리\n중단발", "두상·모질 밸런스 먼저", "sq_inward",
                       "기본 '단발' 기준엔 안 맞을 수 있어요.\n두상과 모질 밸런스를 계산하면\n오히려 중단발이 더 어울릴 때가 많아요.\n길이는 계산의 결과예요."),
        ],
    )
