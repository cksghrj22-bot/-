...
def build(clip, A, B, h1, h2, outname):
    # ⚠️ 반드시 프록시-정렬 전사(diar_{clip}p.json) 사용 — 원본 diar은 프록시보다 길어 자막이 밀린다
    # (2026-07-27 실증). 화자ID는 전사마다 바뀌므로 교사(창엽)=최다발화 화자로 자동검출.
    from collections import Counter
    w = json.load(open(f"{CY}/diar_{clip}p.json"))
    teacher = Counter(x['spk'] for x in w).most_common(1)[0][0]
    words = [(x['s'] - A, x['t']) for x in w
             if A <= x['s'] <= B and x.get('spk') == teacher]
    cues = []; cur = []
    for lt, t in words:
...
# ⚠️ 창(A,B)은 프록시-정렬 전사(diar_{clip}p.json) 기준 시각이다(원본 diar 아님).
CFG = [
    ("9005", 43.0, 66.6, "머리 부드럽게 하는 법", "질감처리의 비밀", "창엽쇼츠_01_질감처리"),
    ("9005", 66.9, 94.5, "머리 무게감 빼는 법", "디스커넥션의 원리", "창엽쇼츠_02_무게감"),
    ("9006", 73.5, 99.0, "짧게 자르면 안 되는 이유", "고객 모질을 봐야 하는 법", "창엽쇼츠_03_모질"),
    ("9006", 376.5, 400.0, "여성스러운 커트의 비밀", "시스루의 원리", "창엽쇼츠_04_시스루"),
    ("9006", 475.0, 499.0, "슬라이싱과 틴닝의 차이", "움직임을 만드는 커트", "창엽쇼츠_05_슬라이싱"),
    ("9006", 276.0, 300.5, "질감처리 명암 읽는 법", "자를 곳과 남길 곳", "창엽쇼츠_06_명암"),
    ("9006", 53.5, 72.0, "머리를 솜사탕처럼", "가볍게 만드는 법", "창엽쇼츠_07_솜사탕"),
]
