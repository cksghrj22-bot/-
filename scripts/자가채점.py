...
def score(txt, name=""):
    ...
    # ⑮ 양방향 — SNS는 소통이다. 단정으로 닫지 않고 시야를 열거나 궁금하게 만든다.
    #    형 지시 2026-08-12: "내 말로 단정하지 말고 이것으로부터 시야가 열리거나
    #    궁금증이 생기거나 할 수 있도록 하자. SNS는 소통이 기본이다. 소통은 양방향이다."
    끝 = L[-1] if L else ""
    열림 = 끝.rstrip().endswith("?") or any(
        k in 끝 for k in ["궁금", "어떨까", "한번 보세요", "찾아보", "무엇일까", "볼까요"])
    접점 = len(re.findall(r"당신|여러분|이신 분|하시는 분|가시는 분|세요|실까요|나요\?|까요\?", txt))
    add("⑮ 양방향(소통)", 열림 and 접점 >= 3,
        ("마지막 줄이 단정이다 — 시야를 열거나 되물어야 한다: 「%s」" % 끝[:20]) if not 열림
        else ("시청자를 부르는 지점 %d회 — 3회 이상 필요" % 접점) if 접점 < 3
        else "열고 닫음 · 시청자 접점 %d회" % 접점)
