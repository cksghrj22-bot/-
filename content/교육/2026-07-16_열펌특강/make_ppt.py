...
def prism(s, cx, cy, size=1.0, rays=None):
    """분해 도식: 흰 빛 한 줄 → 삼각 프리즘 → 여러 갈래로 갈라짐."""
    rays = rays or [("열", ACC), ("약", ACC2), ("커트", INK), ("공간", SUB)]
    h = 1.15 * size; w = 1.0 * size
    tri = [(cx, cy - h/2), (cx - w/2, cy + h/2), (cx + w/2, cy + h/2)]
    poly(s, tri, fill=CARD, line_c=INK, w=2.4, close=True)
    # 입사광 (왼쪽 흰 빛 한 줄 — 짧게)
    line(s, cx - 1.55*size, cy, cx - w/2 + 0.1*size, cy, color=INK, w=2.2)
    txt(s, cx - 1.7*size, cy - 0.52, 1.4, 0.4, "열펌", size=16, color=INK, bold=True, align=PP_ALIGN.CENTER)
    # 출사광 (오른쪽 여러 갈래)
    ...
    for i, (lab, c) in enumerate(rays):
        ...
        txt(s, ex + 0.08, ey - 0.22, 1.3, 0.44, lab, size=17, color=c, bold=True)
...
# ══ 1. 표지 — 프리즘(분해) ══
s = slide("오프닝. 워드월 직후. 화면엔 '열펌'이란 한 줄기 빛이 프리즘을 지나 열·약·커트·공간으로 갈라진다. '오늘은 이걸 쪼갤 겁니다'로 시작.")
