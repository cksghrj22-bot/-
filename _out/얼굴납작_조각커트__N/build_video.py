...
PLAN = [
 ("IMG_2721.MOV", 0.60, 4.0, 0.42, 0.06,
   [("설정","거울 봤을 때,"),
    ("설정","머리가 혹은 얼굴이"),
    ("설정","납작해 보이지 않나요?"),
    ("결론","커트 문제일 확률이 높습니다.")]),   # 32자 한 박스 → 계단 3박스. 전부 58 유지
 ("IMG_2720.MOV", 0.00, None, 0.45, 0.55,   # 차노 「그냥 쭈욱 길게」 → 13.08초 원본 전체
   [("설정","동양인의 두상은"),("설정","대부분 납작하기때문에"),
    ("결론","이점에 유의하여 커트하면"),("결론","충분히 보완 할 수 있습니다.")]),
 ("IMG_2724.MOV", 0.00, None, 0.34, 0.05,
   [("설정","그래서 두상, 헤어를 조각함으로써"),
    ("시안","얼굴의 단점은 가리고 장점은 커버합니다.")]),
 ("IMG_2725.MOV", 0.00, None, 0.42, 0.05,
   [("설정","똑같은 단발이라도"),("결론","이부분을 고려한 단발과"),
    ("결론","아닌 단발은 움직일때 다릅니다.")]),
