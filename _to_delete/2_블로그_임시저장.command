...
for d in _publish_jobs/blog_parsed/*/; do
  ...
  t=$(cat "$d/title.txt" 2>/dev/null)
  echo "  $i) $t"
  JOBS+=("$n")
  i=$((i+1))
done

if [ ${#JOBS[@]} -eq 0 ]; then echo "올릴 글이 없습니다."; read -p "엔터 " _; exit 1; fi
...
IDX=$((SEL-1))
JOBNAME="${JOBS[$IDX]}"
if [ -z "$JOBNAME" ]; then echo "잘못된 번호"; read -p "엔터 " _; exit 1; fi

echo ""
echo "▶ 선택: $(cat "_publish_jobs/blog_parsed/$JOBNAME/title.txt")"
echo ""
...
JOB="$JOBNAME" node scripts/naver_blog_save.mjs
